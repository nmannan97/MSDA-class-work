...
class DailyData:
    ...
    def __str__(self):
        return f"{self.date}: Close=${self.close:.2f}, Volume={self.volume}"
...
class Stock:
    ...
    def sell(self, amount):
        self._shares = max(0, self._shares - amount)
    ...
    def fetch_data(self, period="1mo"):
        ...
        try:
            ...
            for index, row in hist.iterrows():
                ...
                self.dataList.append(DailyData(date, close, volume))

            print(f"✅ Loaded {len(self.dataList)} days of data for {self.symbol}")

        except Exception as e:
            print(f"❌ Failed to fetch data for {self.symbol}: {e}")

    def show_data(self, limit=5):
        print(f"\n📊 Last {limit} records for {self.symbol}:")
        for data in self.dataList[-limit:]:
            print(data)
